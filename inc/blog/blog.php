			...
			if ( is_archive() || is_home() || is_search() ) {
				...
				if ( 'blog-layout-2' == $blog_layout ) {
					...
					if ( in_array( 'kmt-col-sm-12', $classes ) ) {
					 ...
					 $desktop_columns = ! empty( $blog_grids['desktop'] ) ? ' kmt-col-md-' . strval( 12 / $blog_grids['desktop'] ) : ' kmt-col-sm-12';
...
if ( ! function_exists( 'kemet_blog_get_post_meta' ) ) {
	...
	function kemet_blog_get_post_meta() {
		...
		if ( 'post' == get_post_type() && is_array( $post_meta ) && $enable_meta ) {
			...
			if ( ! empty( $output_str ) ) {
				echo apply_filters( 'kemet_blog_post_meta', '<div class="entry-meta">' . $output_str . '</div>', $output_str ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
}
