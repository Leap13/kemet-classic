...
const SpacingComponent = (props) => {
    ...
    const renderInputHtml = (device, active = "") => {
        const { linked_choices, id, title, choices, inputAttrs, connected } =
            props.params;
        ...
        let linkHtml = linked_choices ? (
            <li
                ...
                className={`kmt-spacing-input-item-link ${disconnectedClass}`}
            >
                <span
                    title={title}
        ...
        return (
            <ul
                key={device}
                className={`kmt-spacing-wrapper ${device} ${active}`}
            >
                {htmlChoices}
                {linkHtml}
            </ul>
