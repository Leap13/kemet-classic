...
if ( ! class_exists( 'Kemet_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Kemet_WebFont_Loader {
		...
		public function __construct( $url = '' ) {
			...
			add_action( 'kemet_delete_fonts_folder', array( $this, 'kemet_delete_fonts_folder' ) );
		}
		...
		public function get_local_files_from_css() {
			...
			if ( $change ) {
				...
				update_site_option( 'kemet_downloaded_font_files', $stored );
			}
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'kemet_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'kemet_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'kemet_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'kemet_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function schedule_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_scheduled( 'kemet_delete_fonts_folder' ) && ! wp_installing() ) {
					wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'kemet_delete_fonts_folder' );
				}
			}
		}
		...
		public function delete_fonts_folder() {
			delete_site_option( 'kemet_local_font_files' );
...
if ( ! function_exists( 'kemet_get_webfont_url' ) ) {
	/**
	 ...
	 */
	function kemet_get_webfont_url( $url, $format = 'woff2' ) {
