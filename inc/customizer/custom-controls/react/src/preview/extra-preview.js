...
let btnSelector = 'button, .button, .kmt-button, input[type=button], input[type=reset], input[type="submit"], .wp-block-button a.wp-block-button__link, .wp-block-search button.wp-block-search__button';
...
wp.customize(settingName('divider-item-style'), function (value) {
    value.bind(function (newValue) {
        const allDivider = Array.from(document.querySelectorAll('.kmt-divider-container'));
...
const editBtn = function () {
    setTimeout(() => {
        const allEditButtons = Array.from(document.querySelectorAll('.customize-partial-edit-shortcut:not(.kmt-custom-partial-edit-shortcut):not(.kmt-custom-partial-edit)'));
    ...
    const allButtons = Array.from(document.querySelectorAll('.site-builder-focus-item .item-customizer-focus, .kmt-item-focus .edit-buidler-item'));
    if (allButtons.length > 0) {
        allButtons.map(element => {
            element.addEventListener('click', (e) => {
                ...
                const item = e.target.classList.contains('item-customizer-focus') ? e.target.closest('.site-builder-focus-item') : e.target.closest('.kmt-item-focus');
