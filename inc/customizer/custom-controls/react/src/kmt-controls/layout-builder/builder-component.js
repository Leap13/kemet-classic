...
const BuilderComponent = (props) => {
    ...
    let choices = props.params.choices ? props.params.choices : [];
            ...
            for (const [zoneKey, zoneValue] of Object.entries(value)) {
                for (let zoneItem of zoneValue) {
                    if (zoneItem === item.id) {
                        ...
                        prevITems["restrictValue"] =
                            staleValue[restrictRow][restrictZone];
                        setState((prevState) => ({
                            ...prevState,
                            revertDrag: true,
                            prevItems: prevITems,
    ...
    const updateRowLayout = () => {
        document.addEventListener("KemetUpdateFooterColumns", function (e) {
            ...
            if (newParams.layouts[e.detail] !== undefined) {
                newParams.layouts[e.detail] = wp
                    .customize(getSettingId(e.detail + "-footer-layout"))
                    .get();
                newParams.columns[e.detail] = wp
                    .customize(getSettingId(e.detail + "-footer-columns"))
                    .get();
