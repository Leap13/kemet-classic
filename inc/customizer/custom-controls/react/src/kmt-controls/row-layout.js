...
import RowComponent from './layout-builder/row-component';
...
const BuilderComponent = props => {
    ...
    let choices = (KemetBuilderCustomizerData && KemetBuilderCustomizerData.choices && KemetBuilderCustomizerData.choices[controlParams.group] ? KemetBuilderCustomizerData.choices[controlParams.group] : []);


    const component_track = props.customizer.control('kemet-settings[cloned-component-track]').setting;
    const prevItems = [];
    ...
    if ("kemet-settings[kemet-desktop-items]" === controlParams.group || "kemet-settings[header-mobile-items]" === controlParams.group) {
    ...
    const updateValues = (value, row = '') => {
        ...
        if ('popup' === row) {
            let popup_control = props.customizer('kemet-settings[header-mobile-popup-items]');
    ...
    const isCloneEnabled = () => {
        ...
        KemetBuilderCustomizerData.component_limit = parseInt(KemetBuilderCustomizerData.component_limit);

        const customizerBuilderchoices = (KemetBuilderCustomizerData && KemetBuilderCustomizerData.choices && KemetBuilderCustomizerData.choices[controlParams.group] ? KemetBuilderCustomizerData.choices[controlParams.group] : []);
        ...
        Object.keys(customizerBuilderchoices).forEach(function (choice) {
            ...
            if (builderDataChoice.hasOwnProperty('builder') && builderDataChoice.hasOwnProperty('type')) {
                ...
                if (component_count[builderComponentType] < KemetBuilderCustomizerData.component_limit) {
                    isToClone = true;
                } else {
    ...
    const cloneItem = (item, row, zone) => {
        ...
        if (sessionStorage.getItem('Kemet-builder-clone-in-progress')) {
            return;
        }
        ...
        if (parseInt(cloneIndex) > parseInt(KemetBuilderCustomizerData.component_limit)) {
            return;
        }
        ...
        KemetBuilderCustomizerData.choices[controlParams.group][cloneTypeId] = cloneData;

        sessionStorage.setItem('kemet-builder-clone-in-progress', true);

        var event = new CustomEvent('KemetBuilderCloneSectionControls', {
            'detail': {
    ...
    const onDragEnd = (row, zone, items) => {
...

        {
            items.length > 0 && items.map(item => {
                ...
                if (('popup' === row && (("kemet-settings[header-desktop-items]" === controlParams.group && itemIncludesMenu && 'mobile-menu' !== item.id) || 'mobile-trigger' === item.id)) || 'popup' !== row && 'mobile-menu' === item.id) {
