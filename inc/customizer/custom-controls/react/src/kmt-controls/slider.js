...
class ResponsiveSliderComponent extends Component {
    ...
    handleReset = (e) => {
        ...
        if (this.responsive) {
            let defUnit =
                    this.state.defaultVal[`${this.state.currentDevice}-unit`],
    ...
    render() {
        ...
        let suffixContent = suffix ? (
            <span class="kmt-range-unit">{suffix}</span>
        ) : null;
        ...
        let unitHTML = units.map((unit) => {
            let unit_class;
            if (this.responsive) {
                this.state.initialState[`${this.state.currentDevice}-unit`] ===
                unit
                    ? (unit_class = "active")
                    : (unit_class = "");
            } else {
            ...
            return (
                <li className={`single-unit ${unit_class}`} data-unit={unit}>
                    <span
                        className="unit-text"
                        onClick={(value) =>
        ...
        const onChangeInput = (event) => {
            ...
            if (dataAttributes.min < -0.1) {
                newValue > dataAttributes.max
                    ? this.updateValues(dataAttributes.max)
                    : newValue < dataAttributes.min && newValue !== "-"
        ...
        let sliderValue = this.responsive
            ? this.state.initialState[this.state.currentDevice]
            ...
            <label htmlFor="">
                ...
                <div className="wrapper">
                    <div className={`input-field-wrapper active`}>
                        ...
                        <ul className="kmt-slider-units">{unitHTML}</ul>
                    </div>

                    <button
                        className="kmt-slider-reset"
                        disabled={
                            this.state.initialState === this.state.defaultVal
                                ? true
                                : false
                        }
