...
class KemetColorPickerControl extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            supportGradient:
                undefined === __experimentalGradientPicker ? false : true,
        };
    }
    ...
    render() {
        const { refresh, isVisible, supportGradient, backgroundType } =
            this.state;

        const { allowGradient, allowImage } = this.props;
                        ...
                        {defaultColorPalette.map((color, index) => (
                            <li
                                ...
                                className={classnames({
                                    active: this.props.color === color,
                                })}
                                onClick={() =>
                                    this.onPaletteChangeComplete(color)
                                }
                            >
            ...
            <>
                <div className="color-button-wrap">
                    <Button
                        className={
                            isVisible
                                ? "kemet-color-icon-indicate open"
                                : "kemet-color-icon-indicate"
                        }
                        data-background-type={this.props.backgroundType}
                        onClick={() => {
                            isVisible ? this.toggleClose() : toggleVisible();
                        }}
                        style={{
                            backgroundColor: this.props.color,
                            "--background-position": this.props
                                .backgroundPosition
                                ? ` ${Math.round(
                                      parseFloat(
                                          this.props.backgroundPosition.x
                                      ) * 100
                                  )}% ${Math.round(
                                      parseFloat(
                                          this.props.backgroundPosition.y
                                      ) * 100
                                  )}%`
                                : null,
                            "--background-image":
                    ...
                    >
                        <i className="kmt-tooltip-top">{this.state.text}</i>
                    </Button>
            ...
            <>
                <div className="kmt-control kmt-image-actions">
                    <MediaUpload
                        ...
                        allowedTypes={["image"]}
                        value={
                            this.props.media && this.props.media
                                ? this.props.media
                                : ""
                        }
                        render={({ open }) => (
                            <>
                                ...
                                {this.props.media &&
                                    this.props.backgroundType === "image" && (
