...
class Kemet_Footer_Button_Customizer extends Kemet_Customizer_Register {
	...
	public function register_options( $options ) {
		self::$button_items = apply_filters( 'kemet_footer_button_items', array( 'footer-button-1', 'footer-button-2' ) );
		$register_options   = array();
		foreach ( self::$button_items as $button ) {
			$prefix         = $button;
			$num            = explode( 'footer-button-', $prefix )[1];
			$selector       = '.' . $button;
		...
		foreach ( self::$button_items as $button ) {
			...
			$button_section = array(
				'section-' . $prefix => array(
					'priority' => 60,
					'title'    => sprintf( __( 'Footer Button %s', 'kemet' ), $num ),
