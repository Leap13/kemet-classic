<?php
/**
 * Footer Html Options for Kemet Theme.
 *
 * @package     Kemet
 * @author      Kemet
...
class Kemet_Footer_Html_Customizer extends Kemet_Customizer_Register {
	...
	public function register_options( $options ) {
		self::$html_items = apply_filters( 'kemet_footer_html_items', array( 'footer-html-1', 'footer-html-2' ) );
	...
	public function register_sections( $sections ) {
		...
		foreach ( self::$html_items as $html ) {
			$prefix       = $html;
			$num          = explode( 'footer-html-', $prefix )[1];
			$html_section = array(
				'section-' . $prefix => array(
					'priority' => 60,
					'title'    => sprintf( __( 'Footer Html %s', 'kemet' ), $num ),
	...
	public function add_partials( $partials ) {
		foreach ( self::$html_items as $html ) {
			...
			$partials[ $prefix . '-text' ] = array(
				...
				'render_callback'     => array( Kemet_Footer_Markup::get_instance(), 'render_html_' . $num ),
			);
		}
...
new Kemet_Footer_Html_Customizer();
